...
var dataFiles = [
    './data/baer/baer_current_charts.json', './data/bray/bray_current_charts.json', './data/garcia/garcia_current_charts.json', './data/gardner/gardner_current_charts.json', './data/hickenlooper/hickenlooper_current_charts.json', './data/zornio/zornio_current_charts.json'
];

$.when.apply($, dataFiles.map(function(url) {
    return $.ajax(url);
})).done(function() {
    var values = [];
    var keys = ['Baer', 'Bray', 'Garcia', 'Gardner', 'Hickenlooper', 'Zornio'];
    ...
    for (var i = 0; i < arguments.length; i++) {
        values.push(arguments[i][0].slice(-1)[0].total_receipts_ytd);
    }
    ...
    var quarterlyMoneyChart = c3.generate({
        ...
        data: {
            ...
            // },
            columns: merged,
            //names: ['Baer', 'Bray', 'Garcia', 'Gardner', 'Hickenlooper', 'Zornio'],
            // names: {
